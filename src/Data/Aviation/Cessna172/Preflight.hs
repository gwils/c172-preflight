...
import Diagrams.TwoD.Text(Text, text, alignedText)
...
rr :: (Renderable (Text Double) b, Renderable (Path V2 Double) b) =>
  QDiagram b V2 Double Any
rr =
  text "tsdfgsdfgsdfgdfsgdfgdfgdfgdest"

renderResult ::
  (Renderable (Text Double) b, Renderable (Path V2 Double) b) =>
  QDiagram b V2 Double Any
renderResult = 
  vcat' (with & sep .~ 5) [renderAxis result, rr, renderAxis result]
